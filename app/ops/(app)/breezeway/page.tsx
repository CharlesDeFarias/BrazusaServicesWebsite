  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h1 className="text-lg font-semibold">Breezeway data</h1>
            ...
            return (
              <section key={d} className="space-y-1">
                <h2 className="text-sm font-medium text-neutral-300">
                  {d} <span className="text-neutral-500">({wd})</span>{' '}
                  <span className="text-neutral-600">· {byDate.get(d)!.length}</span>
                </h2>
                <Card className="divide-y divide-neutral-800/60 text-sm">
                  {byDate.get(d)!.map((t, i) => (
                    <div key={i} className="flex justify-between gap-3 px-3 py-1.5">
                      <span className="text-neutral-300 truncate">{shortProp(t[0])}</span>
                      <span className="text-neutral-500 whitespace-nowrap text-xs">{t[2]}</span>
                    </div>
                  ))}
                </Card>
              </section>
