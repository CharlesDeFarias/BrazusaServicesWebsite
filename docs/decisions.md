...
**Status:** Phase 3 in progress. `copy.txt` rewritten. Design audit complete. Accordion copy refined and committed. Hero copy updated 2026-04-20 (new h1, collapsed body, 10 differentiators, new microcopy). Segment copy layer scaffolded (`lib/copy/brazusa-cleaning/`) -- Hero wired, segment stubs in place. In progress: accordion copy expansion (ChatGPT brief sent, return pending). Next: fill segment stubs with approved per-segment hero copy once accordion pass is complete.
