...
**Decision:** `.grain::after` uses `z-index: -1` (within the `isolation: isolate` stacking context on `.grain`). The old `.grain > * { position: relative; z-index: 2; }` rule has been removed entirely.
**Why:** The `.grain > *` rule had specificity 0-1-0, same as Tailwind's `.absolute` class, but appeared later in the stylesheet (after `@import "tailwindcss"`). At equal specificity, source order wins — so `.grain > *` silently overrode `position: absolute` on any direct child of a `.grain` section. The only visible victim was FinalCTA's gold glow div (`w-80 h-80`), which became `position: relative` in the flow — a 320px nearly-invisible block appearing before the FinalCTA content. The fix: `z-index: -1` on `::after` within the `isolation: isolate` stacking context paints the grain texture above section backgrounds but below all children (positioned or not) without needing to touch child positioning at all.
**Constraints:** Do not re-add `.grain > *` with `position: relative`. When adding absolute-positioned decorative elements inside `.grain` sections, they will correctly stay absolute. Any element that truly needs `position: relative` for layout should have the `relative` Tailwind class directly on it.
