        ...
        <div className="mt-14 grid gap-6 lg:grid-cols-3">
          {r.items.map((item, i) => (
            <Reveal
              key={i}
              delay={(i % 3) * 0.08}
              className="flex flex-col justify-between rounded-3xl border border-forest/10 bg-ivory p-8 shadow-sm shadow-forest/5"
            >
              <div>
                <Stars />
                <p className="mt-5 font-display text-xl italic leading-snug text-charcoal/85">
                  “{item.quote}”
                </p>
              </div>
              <div className="mt-7 border-t border-forest/10 pt-5">
                <p className="font-medium text-forest">{item.author}</p>
                <p className="text-sm text-charcoal/55">{item.detail}</p>
              </div>
            </Reveal>
          ))}
        </div>
