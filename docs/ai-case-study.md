...
13. **Second Claude Design pass and full redesign implementation** - the site went through a substantially deeper design review pass that produced a 16-task implementation handoff. This pass was intentionally disjointed across multiple sessions due to context constraints: Claude Design feedback was collected in one conversation, then handed off to Claude Code in a separate session via a structured prompt generated by the prompt-engineering-advisor skill. The handoff document specified a new locked section order, two new components (CalloutBand, TrustStats), three retired components (HowItWorks, MobileCTABar, QuickContact), Testimonials replaced from carousel to category accordion, FinalCTA merged with QuickContact contact information, and a section label bar pattern applied to every section. Five open questions were sent back to Claude Design before implementation started - all five were answered and resolved. Gold discipline was suspended by Charles's explicit preference (too many concurrent elements to evaluate in isolation; deferred to a final pass). This session also marked the point where Charles upgraded to Claude Max, reflecting a substantial increase in reliance on Claude for the project.
...
**Claude Design as a disjointed workflow.** Using Claude Design as a design review layer works well when the review and implementation happen in tight sequence. When they span multiple sessions with context gaps - as they did in the second Brazusa pass - the handoff needs to be more structured than a simple copy-paste. The solution was generating a bridging prompt with the prompt-engineering-advisor skill before implementation, and routing five unresolved questions back to Claude Design before starting any code. The lesson: treat cross-model handoffs as formal artifacts, not informal notes, particularly when there's a context break between the review and the build.
...
- **Code-review skill.** A personal Claude skill that checks coding standards compliance, catches basic errors, and audits file architecture for messiness - loose files, misplaced components, line-count threshold violations. Currently this guidance lives in CLAUDE.md as text; a skill would make it executable on demand.
...
- **Component render test coverage.** Full component render tests using jsdom and React Testing Library are not possible with the current Vitest node environment. A package install is required. Deferred until the site is more finished - add in a single pass across all components rather than piecemeal.
...
- ~~Second Claude Design pass + full redesign implementation~~ - done (commit efdef3a, April 29 2026). New section order locked, CalloutBand and TrustStats added, HowItWorks/MobileCTABar/QuickContact removed, Testimonials rewritten as category accordion, FinalCTA merged with contact info, section label bar pattern applied site-wide. Gold discipline deferred to a final review pass.
